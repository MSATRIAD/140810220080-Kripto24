...
void enc(const string& P, int n) {
    ...
    plaintext.resize(size, 'X');  // Tambahkan padding 'X' jika diperlukan
    ...
    for (int i = 0; i < size; i += n) {
        ...
        for (int j = 0; j < n; j++) {
            C[i + j] = (temp[j] % 26 + 26) % 26 + 65;  // Pastikan hasil dalam rentang A-Z
        }
    }
...
void dec(const string& P, int n) {
    ...
    for (int i = 0; i < P.size(); i += n) {
        ...
        for (int j = 0; j < n; j++) {
            plain[i + j] = (temp[j] % 26 + 26) % 26 + 65;  // Pastikan hasil dalam rentang A-Z
        }
    }
...
vector<int> StringConverter(const string& P) {
    ...
    for (char c : P) {
        if (isalpha(c)) {
            ...
            plain.push_back(c); // Menyimpan karakter non-alfabet seperti spasi
        }
    }
    return plain;
}
...
string IntConverter(const vector<int>& C) {
    ...
    for (int i = 0; i < C.size(); i++) {
        if (C[i] >= 0 && C[i] < 26) {
            cipher[i] = static_cast<char>(C[i] + 65);  // Konversi ke huruf kapital
        } else {
            cipher[i] = '?';  // Jika nilai di luar rentang
        }
        cout << "Cipher[" << i << "] = " << cipher[i] << " (Int: " << C[i] << ")" << endl; // Debug print
    }
    return cipher;
}
...
int Determinan(const vector<vector<int>>& K, int n) {
    int det = 0;
    if (n == 1) return K[0][0];
    if (n == 2) return (K[0][0] * K[1][1] - K[0][1] * K[1][0]) % 26;
    vector<vector<int>> temp(n, vector<int>(n));
    int sign = 1;
    ...
    for (size_t i = 0; i < plain.size(); i += n) {
        ...
        for (int r = 0; r < n; r++) {
            for (int c = 0; c < n; c++) {
                ...
                if (tempMatrix[r][c] < 0) tempMatrix[r][c] += 26;
            }
        }
